<?php
	...
	function tambah_data($data, $files){
        ...
        $query = "INSERT INTO mahasiswa VALUES(null, '$npm', '$foto_mahasiswa', '$nama_mahasiswa', '$gender', '$tanggal_lahir', '$alamat', '$jurusan')";
	...
	function ubah_data($data,$files){
        ...
        $query = "UPDATE mahasiswa SET npm='$npm', foto_mahasiswa='$foto_mahasiswa', nama_mahasiswa='$nama_mahasiswa', gender='$gender', tanggal_lahir='$tanggal_lahir', alamat='$alamat', jurusan='$jurusan' WHERE id_mahasiswa ='$id_mahasiswa';";
        $sql = mysqli_query($GLOBALS['conn'], $query);
    ...
    function ubah_nilai($data) {
        ...
        $triggerQuery = "UPDATE nilai_mahasiswa SET ipk = (tugas * 0.20 + uts * 0.30 + uas * 0.30 + tugas_akhir * 0.20) / 25,
                        predikat = CASE
    ...
    function tambah_peserta_kelas($kode_kelas) {
        // Retrieve the list of students who are not yet members of the specified class
        $query = "SELECT * FROM mahasiswa WHERE npm NOT IN (SELECT npm FROM nilai_mahasiswa WHERE kode_kelas != '$kode_kelas')";
        $result = mysqli_query($GLOBALS['conn'], $query);
    
        if ($result) {
            return $result;
        } else {
            return "Error fetching data: " . mysqli_error($GLOBALS['conn']);
        }
    }
